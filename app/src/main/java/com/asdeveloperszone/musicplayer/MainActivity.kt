...
class MainActivity : AppCompatActivity(), ServiceConnection {
    ...
    private var currentTab = 0 // 0=Songs, 1=Albums, 2=Artists, 3=Favorites
    ...
    private fun selectTab(tab: Int) {
        currentTab = tab
        listOf(tabSongs, tabAlbums, tabArtists, tabFavorites).forEach {
            it.setBackgroundResource(R.drawable.bg_tab_inactive)
            it.setTextColor(0xAAFFFFFF.toInt())
        }
        val active = when (tab) {
            0 -> tabSongs; 3 -> tabFavorites; else -> tabSongs
        }
    ...
    private fun showSleepTimer() {
        val options = arrayOf("15 minutes", "30 minutes", "45 minutes", "60 minutes",
            "90 minutes", if (SleepTimerManager.isRunning) "Cancel Timer (${SleepTimerManager.getFormattedTime()})" else "Cancel Timer")
        AlertDialog.Builder(this, R.style.SortDialogTheme)
    ...
    private fun startTimer(minutes: Int) {
        SleepTimerManager.onFinish = { svc?.stop(); miniPlayer.visibility = View.GONE }
    ...
    private fun checkAudioPermission() {
        ...
        if (ContextCompat.checkSelfPermission(this, perm) == PackageManager.PERMISSION_GRANTED)
            ...
            ActivityCompat.requestPermissions(this, arrayOf(perm), REQ_AUDIO)
    }

    override fun onRequestPermissionsResult(rc: Int, p: Array<String>, r: IntArray) {
        super.onRequestPermissionsResult(rc, p, r)
        if (rc == REQ_AUDIO && r.isNotEmpty() && r[0] == PackageManager.PERMISSION_GRANTED)
            loadSongs()
    }
                ...
                contentResolver.query(
                    ...
                    while (c.moveToNext()) {
                        try {
                            ...
                            val aid = c.getLong(cp)
                            if (dur > 0) list.add(Song(id,
                                c.getString(ct) ?: "Unknown",
                                c.getString(ca) ?: "Unknown Artist",
                                c.getString(cb) ?: "Unknown Album",
                                dur,
