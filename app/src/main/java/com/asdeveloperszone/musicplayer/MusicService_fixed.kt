...
class MusicService : Service() {
    ...
    fun togglePlayPause() {
        try {
            mediaPlayer?.let {
                ...
                getCurrentSong()?.let { song -> updateNotificationAsync(song) }
            }
    ...
    private fun playCurrent() {
        ...
        try {
            ...
            // Show notification immediately with text, then update with album art
            startForeground(NOTIFICATION_ID, buildNotification(song, null))
            updateNotificationAsync(song)
        } catch (e: Exception) {
    ...
    private fun updateNotificationAsync(song: Song) {
        Thread {
            ...
            // Build and post notification on main thread
            mainHandler.post {
                try {
    ...
    private fun buildNotification(song: Song, albumArt: Bitmap?): Notification {
        ...
        val builder = NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setSubText(song.getDurationFormatted())
